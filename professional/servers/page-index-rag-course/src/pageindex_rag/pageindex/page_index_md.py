...
async def get_node_summary(node, summary_token_threshold=200, extractive_threshold=2000,
                           model=None, semaphore=None):
    node_text = node.get('text')
    num_tokens = count_tokens(node_text, model=model)

    # Tier 1: Small nodes — use raw text as-is
    if num_tokens < extractive_threshold:
        return node_text

    # Tier 2: Mid-tier nodes — fast extractive summary (no LLM)
    if num_tokens < summary_token_threshold:
        logger.warning(
            "DIAG EXTRACTIVE: title=%r tokens=%d",
            node.get('title', '')[:60], num_tokens
        )
        return extractive_summary(node_text, num_sentences=3)

    # Tier 3: Large nodes — LLM abstractive summary
    logger.warning(
...
async def generate_summaries_for_structure_md(structure, summary_token_threshold, model=None):
    from ..llm import _get_summary_model, _get_summary_concurrency, _get_extractive_threshold
    summary_model = model if model is not None else _get_summary_model()
    concurrency = _get_summary_concurrency()
    extractive_thresh = _get_extractive_threshold()
    semaphore = asyncio.Semaphore(concurrency) if concurrency else None
    logger.warning("DIAG generate_summaries: threshold=%d extractive=%d concurrency=%s model=%s",
                   summary_token_threshold, extractive_thresh, concurrency, summary_model)

    nodes = structure_to_list(structure)
    tasks = [get_node_summary(node, summary_token_threshold=summary_token_threshold,
                              extractive_threshold=extractive_thresh,
                              model=summary_model, semaphore=semaphore) for node in nodes]
