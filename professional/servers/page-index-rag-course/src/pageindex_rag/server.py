...
def get_rag_guide() -> dict:
    ...
    return {
        ...
        "query_routing": {
            ...
            "executive compensation": "Item 11, DEF 14A proxy (salary, bonus, stock/option awards)",
        ...
        "important": (
            "For any question about a specific public company's financials, operations, "
            "risks, insiders, or governance: ALWAYS search indexed filings FIRST before "
            "falling back to web search. SEC filings are the authoritative primary source. "
            "Web search should only supplement with real-time data not in filings "
            "(e.g., today's stock price, breaking news). "
            "Call list_documents() to see what is already indexed before answering."
        ),
        "rules": [
            "ALWAYS check_company_indexed() before fetch_company_filings()",
            "ALWAYS search indexed filings BEFORE falling back to web search for company-specific data",
...
def search_with_citations(query: str, doc_id: str = "", max_results: int = 5) -> str:
    """Search SEC filings for financial data, risk factors, insider transactions
    (10b5-1 plans), share repurchases, executive compensation, legal proceedings,
    segment revenue, debt/equity details, beneficial ownership, related party
    transactions, and any other company-specific disclosure.

    This is the AUTHORITATIVE SOURCE for public company data — prefer this over
    web search for any question whose answer likely lives in a 10-K, 10-Q, proxy,
    or other SEC filing.

    USE WHEN: Any question about a company's financials, operations, risks,
    insiders, governance, or legal matters. Also use for verifiable references
    and precise doc_id + node_id pairs for follow-up reads.
