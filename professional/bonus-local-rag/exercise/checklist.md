# Bonus Module: Evaluation Checklist
...
- [ ] **Ollama is running.** `ollama ls` shows `nomic-embed-text` and
  your chosen text model (`qwen3.5:0.8b`, `qwen3.5:2b`, `qwen3.5:4b`,
  or `qwen3.5:9b` depending on your machine's RAM).
...
- [ ] **Document downloaded and cleaned.** A text file exists in your
  documents folder with readable content and intact section headers.

- [ ] **Section boundaries preserved.** The text file has clear section
  markers (Item 1, Item 1A, PART I, etc.) that the indexer can use.

## Indexing

- [ ] **Indexer runs successfully.** The document is split into sections
  and each section is embedded via Ollama.

- [ ] **Index file created.** A JSON file exists with the section text
  and embeddings stored.

- [ ] **Reasonable section count.** A 10-K filing should produce roughly
  15-40 sections (not 500 tiny chunks or 3 enormous ones).

- [ ] **Re-indexing is skipped.** Running the indexer again on the same
  document detects the existing index and skips re-embedding.
...
- [ ] **Similarity scores make sense.** The top result has a higher
...
- [ ] **Answers are grounded.** The answer is based on the document
  content, not general knowledge. Test by asking something specific
  that only this filing would mention.
...
- [ ] **Error handling for Ollama.** Stop Ollama (`Ctrl+C` or quit
  the app), then call a tool. It should return a helpful error
  message, not crash.
...
- [ ] **Connected to Claude Desktop.** Claude Desktop can call your
  RAG tools alongside your other servers.
...
- [ ] **Cross-server works.** Ask Claude Desktop something that
  combines RAG data with stock data from your Week 1 server:
  "What does Apple's 10-K say about their business, and how is
  AAPL performing today?"

## Data Privacy

- [ ] **All processing is local.** The `data_source` field says
  "local_ollama" — confirming no cloud API was used for document
  processing.

- [ ] **No document content in cloud calls.** When Claude Desktop
  interprets the RAG results, it receives excerpts from your tools —
  the full document was never uploaded anywhere.
