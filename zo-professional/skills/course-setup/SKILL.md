...
4. **Register the MCP servers with Zo.** The bootstrap cannot register MCP servers on the student's behalf — Zo handles that. The bootstrap script prints the exact registration prompt at the end of its run, with the absolute launcher paths interpolated. The prompt format is:
   ...
   specs (use register_user_service with the Command verbatim):
     ...
     1. Service name: spy-tlt-course
        Command:      /home/workspace/Skills/course-setup/scripts/run-spy-tlt-server.sh
        Transport:    stdio MCP

     2. Service name: page-index-rag-course
        Command:      /home/workspace/Skills/course-setup/scripts/run-rag-server.sh
        Transport:    stdio MCP

   After registering, confirm both services are RUNNING (not just registered),
   then list the tools each exposes and confirm page-index-rag-course's
   list_documents returns at least 7 indexed filings (BLK + HOOD).
   ```

   **Why absolute launcher paths matter:** Zo registers user services through `supervisord-user`, which uses exec-style spawning. A command like `cd <dir> && uv run xxx` silently fails because supervisord tries to exec the literal `cd` binary. The launcher scripts (generated by bootstrap at install time) wrap the `cd` and absolute `uv` path internally, so the supervisord command is a single absolute path with no shell constructs.
